        ...
        <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
            <form>
                <div class="flex flex-wrap -mb-8 -mr-6 p-8">
                    <text-input v-model="residentes.nombres" type="text" label="Nombres"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="nombres" name="nombres" :error="errors.nombres" />
                    <!-- v-model="personas.nombres" -->
                    <text-input v-model="residentes.apellidos" type="text" label="Apellidos"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="apellidos" name="apellidos" :error="errors.apellidos" />
                    <!-- v-model="personas.apellidos" -->
                    <text-input v-model="residentes.ci_numero" type="text" label="CI" class="pb-7 pr-6 w-full lg:w-1/2"
                        :id="ci_numero" name="ci_numero" :error="ci_numero" /> <!-- v-model="personas.ci_numero" -->
                    <text-input v-model="residentes.fecha_nacimiento" type="date" label="Fecha de Nacimiento"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="fecha_nacimiento" name="fecha_nacimiento" :error="errors.fecha_nacimiento" />
                    <!-- v-model="personas.fecha_nacimiento" -->
                    <text-input v-model="residentes.telefono" type="text" label="Teléfono"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="telefono" name="telefono" :error="errors.telefono"/>
                    <!-- v-model="persona.telefono" -->
                    <text-input v-model="residentes.edad" type="number" label="edad" class="pb-7 pr-6 w-full lg:w-1/2"
                        :id="edad" name="edad" :error="errors.edad" /> <!-- v-model="persona.edad" -->
                    <select-input class="pb-8 pr-6 w-full lg:w-1/2" label="Sexo" :error="errors.sexo">
                        <option :value="residentes.sexo" /> <!-- :value="persona.sexo" -->
                        <option value="Femenino">Femenino</option>
                        <option value="Masculino">Masculino</option>
                    </select-input> <!-- v-model="personas.sexo" -->
                    <text-input v-model="residentes.direccion" type="text" label="Direccion"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="direccion" name="direccion" :error="errors.direccion" />
                    <!-- v-model="persona.direccion" -->
                    <text-input v-model="residentes.ciudade_id" type="text" label="Ciudad"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="ciudad" name="ciudade_id" :error="errors.ciudade_id" />
                    <!-- v-model="persona.ciudade_id" -->
                    <text-input v-model="residentes.foto" type="text" label="Foto" class="pb-7 pr-6 w-full lg:w-1/2"
                        :id="foto" name="foto" :error="errors.foto" /> <!-- v-model="residente[0].foto" -->
                    <text-input v-model="residentes.fecha_ingreso" type="date" label="Fecha Ingreso"
                        class="pb-7 pr-6 w-full lg:w-1/2" :id="fecha_ingreso" name="fecha_ingreso" :error="errors.fecha_ingreso"/>
                    <!-- v-model="residente[0].fecha_ingreso" -->
                    <select-input class="pb-8 pr-6 w-full lg:w-1/2" label="Estado" :error="errors.estado">
                        <option :value="residentes.estado" />
                        <option value="Activo">Activo</option>
                        <option value="Inactivo">Inactivo</option>
                    </select-input><!-- v-model="residente[0].estado" -->
                </div>
...
export default {
    ...
    setup(props) {
        ...
        const actualizarResidente = () => {
            ...
            residentes.post(route("residentes.update", residentes.id_residente), {
                preserveScroll: true,
