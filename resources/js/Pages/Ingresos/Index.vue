<template>
    <div>
        ...
        <Head title="Ingresos" />
        <h1 class="mb-7 text-3xl font-bold text-cyan-600">Ingresos</h1>
        <div class="flex items-center justify-between mb-6">
            <Link class="btn-nuevo" type="button" :href="route('ingresos.create')">
            <span class="text-white font-bold">Nuevo Ingreso</span>
            </Link>
        </div>
        <div class="bg-white rounded-md shadow overflow-x-auto">
            <table class="w-full whitespace-nowrap text-sm">
                ...
                <tbody>
                    <tr v-for="ingreso in ingresos.data" :key="ingreso.id"
                        ...
                        <td class="border-t py-3">
                            <Link class="mx-1 inline-block" :href="route('ingresos.edit', ingreso.id)">
                            <icon name="edit" class="w-4 h-4 fill-gray-600 hover:fill-cyan-800" />
                            </Link>
                            <button class="mx-1"  @click="eliminarIngreso(ingreso.id)">
...
export default {
    ...
    setup() {
        ...
        function eliminarIngreso(id) {
            ...
            formDelete.delete(route('ingresos.destroy', id));
