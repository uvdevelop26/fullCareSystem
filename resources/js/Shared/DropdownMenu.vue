<template>
  <button type="button" @click="show = true">
    <!-- acá va el menu hamburguesa del dropdown -->
    <slot />
    <!-- acá va el modal -->
    <teleport v-if="show" to="#dropdown"> 
        <div>
            ...
            <div ref="dropdown" style="position: absolute; z-index: 99999" > <!-- @click.stop="show = !autoClose" -->
                <!-- acá va el menu con los enlaces -->
                <slot name="dropdown" />
            </div>
        </div>
    </teleport>
...
export default {
    ...
    watch:{
    //creamos el popper dependiendo de la propiedad show
        show(show){
            if(show){
                this.$nextTick(()=>{
...
//para que el modal desaparezca cuando se aprieta escape
    mounted(){
