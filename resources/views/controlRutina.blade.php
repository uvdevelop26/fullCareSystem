...
<body>
    ...
    <div class="container">
                    ...
                    <table class="table_h_factura">
                        <thead>
                            <th class="headerDatosh titulos">
                                <span class="titulos">Marcación de Rutinas fecha {{ $fecha }} Hogar de Ancianos
                                    "Virgen del
                                    Rosario"</span>
                            </th>
                        </thead>
        ...
        <table class="table_materiales">
            ...
            <tbody>
                @foreach($controlRutinas as $item)
                <tr>
                    @foreach($item->horarioRutina->rutinas as $item2)
                    <td>{{ $item2->residente->persona->nombres }}</td>
                    @endforeach

                    @foreach($item->horarioRutina->rutinas as $item3)
                    <td>{{ $item3->nombre}}</td>
                    @endforeach
                    <td>
                        {{ $item->horarioRutina->hora }}
                    </td>
                    <td>
                        {{ $item->hora }}
                    </td>
